...
New code should use ResearchPlot's project/profile APIs with ordinary
Matplotlib calls.  Optional plotting integrations are imported only by the
...
from .config import PLOT_FORMATS, LegacyStyleWarning
    ...
    def __init__(self, format: str = "ieee") -> None:
        ...
        if key not in PLOT_FORMATS:
            warnings.warn(
                ...
                "is deprecated. Use researchplot.resolve_profile() for strict resolution.",
                DeprecationWarning,
                stacklevel=2,
            )
            key = "ieee"
...
def _prepare(
    ...
    *,
    ...
    warnings.warn(
        "researchplot.plots helpers are deprecated; use the ResearchPlot project/profile APIs "
        "with native Matplotlib instead.",
        FutureWarning,
        stacklevel=3,
    )
